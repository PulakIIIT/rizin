...
RZ_API void plugin_fini(RzAnalysis *analysis) {
	...
	if (p && p->fini && !p->fini(analysis->user)) {
		eprintf("analysis plugin '%s' failed to terminate.\n", p->name);
	}
}
...
RZ_API bool rz_analysis_use(RzAnalysis *analysis, const char *name) {
	...
	if (analysis) {
		rz_list_foreach (analysis->plugins, it, h) {
			...
			analysis->cur = h;
			if (h && h->init && !h->init(&analysis->user)) {
				eprintf("analysis plugin '%s' failed to initialize.\n", h->name);
				return false;
			}
